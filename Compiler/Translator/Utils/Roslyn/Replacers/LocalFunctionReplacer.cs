    ...
    public class LocalFunctionReplacer : ICSharpReplacer
    {
        ...
        public SyntaxNode Replace(SyntaxNode root, SemanticModel model)
        {
            ...
            foreach (var fn in localFns)
            {
                ...
                var isVoid = fn.ReturnType.Kind() == SyntaxKind.VoidKeyword;
                TypeSyntax varType;
                ...
                else if (isVoid)
                {
                    ...
                    else
                    {
                        ...
                        (
                            ...
                            (
                                SyntaxFactory.TypeArgumentList(SyntaxFactory.SeparatedList<TypeSyntax>(fn.ParameterList.Parameters.Select(p => p.Type)))
                            )
                        );
                    }
                }
                else
                {
                    if (fn.ParameterList.Parameters.Count == 0)
                    {
                        varType = SyntaxFactory.QualifiedName(SyntaxFactory.IdentifierName("System"), SyntaxFactory.GenericName("Func").WithTypeArgumentList(SyntaxFactory.TypeArgumentList(SyntaxFactory.SingletonSeparatedList<TypeSyntax>(fn.ReturnType))));
                    }
                    else
                    {
                        varType = SyntaxFactory.QualifiedName
                        (
                            ...
                            (
                                SyntaxFactory.TypeArgumentList(SyntaxFactory.SeparatedList<TypeSyntax>(fn.ParameterList.Parameters.Select(p => p.Type).Concat(new TypeSyntax[] { fn.ReturnType })))
                            )
                        );
                    }
                }
                ...
                var varDecl = SyntaxFactory.VariableDeclaration(varType).WithVariables(SyntaxFactory.SingletonSeparatedList<VariableDeclaratorSyntax>(
                    SyntaxFactory.VariableDeclarator(SyntaxFactory.Identifier(fn.Identifier.ValueText)).WithInitializer
                    (
                        SyntaxFactory.EqualsValueClause(SyntaxFactory.ParenthesizedLambdaExpression(fn.Body).WithParameterList(
                            SyntaxFactory.ParameterList(SyntaxFactory.SeparatedList<ParameterSyntax>(prms))
                        ))
                    )
                )).NormalizeWhitespace();
            ...
            if (updatedClasses.Count > 0)
            {
                root = root.ReplaceNodes(updatedClasses.Keys, (t1, t2) => {
                    ...
                    t1 = t1.ReplaceNodes(updatedBlocks.Keys, (b1, b2) => b1.WithStatements(SyntaxFactory.List<StatementSyntax>(updatedBlocks[b1])));
